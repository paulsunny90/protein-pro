...
const WomenProductsPage = () => {
    ...
    useEffect(() => {
        console.log('Women Products Page - Fetching all products...');
        dispatch(fetchProducts(true));
    }, [dispatch]);

    useEffect(() => {
        console.log('Women Products Page - Total products:', products.length);
        console.log('Women Products Page - Products data:', products);
    }, [products]);
    ...
    // Filter for Women target group
    const womenProducts = products.filter((p: Product) => p.targetGroup === 'Women');
    console.log('Women Products Page - Filtered women products:', womenProducts.length);

    const filteredProducts = selectedType === 'All'
        ? womenProducts
        : womenProducts.filter((product: Product) => product.productType === selectedType);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
                <div className="text-center mb-20 animate-fade-in">
                    <div className="inline-flex items-center space-x-2 bg-emerald-50 text-emerald-700 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest mb-6">
                        ...
                        <span>Empower Her Strength</span>
                    </div>
                    <h1 className="text-5xl md:text-6xl font-black text-slate-900 mb-6 tracking-tighter">Women's Collection</h1>
                    <div className="w-16 h-1.5 bg-emerald-500 mx-auto rounded-full mb-8"></div>
                    <p className="text-lg text-slate-500 max-w-2xl mx-auto font-medium">
                        Scientifically curated wellness and performance nutrition for the modern woman.
                    </p>
                </div>
