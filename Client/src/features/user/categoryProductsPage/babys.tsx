...
const BabyProductsPage = () => {
    ...
    useEffect(() => {
        console.log('Baby Products Page - Fetching all products...');
        dispatch(fetchProducts(true)); // potentially fetch all to filter on client side, or make a specific API call
    }, [dispatch]);

    useEffect(() => {
        console.log('Baby Products Page - Total products:', products.length);
        console.log('Baby Products Page - Products data:', products);
    }, [products]);
    ...
    // Filter for Baby target group and then by selected Type
    const babyProducts = products.filter((p: Product) => p.targetGroup === 'Babys');
    console.log('Baby Products Page - Filtered baby products:', babyProducts.length);

    const filteredProducts = selectedType === 'All'
        ? babyProducts
        : babyProducts.filter((product: Product) => product.productType === selectedType);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
                <div className="text-center mb-20 animate-fade-in">
                    <div className="inline-flex items-center space-x-2 bg-emerald-50 text-emerald-700 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest mb-6">
                        ...
                        <span>Essential Nutrition for Kids</span>
                    </div>
                    <h1 className="text-5xl md:text-6xl font-black text-slate-900 mb-6 tracking-tighter">Kids & Baby Collection</h1>
                    <div className="w-16 h-1.5 bg-emerald-500 mx-auto rounded-full mb-8"></div>
                    <p className="text-lg text-slate-500 max-w-2xl mx-auto font-medium">
                        Gentle, nutrient-dense formulas designed to support healthy growth and development.
                    </p>
                </div>
                ...
                {!loading && sortedProducts.length === 0 && (
                    <div className="text-center py-32 animate-fade-in">
                        ...
                        <h3 className="text-2xl font-black text-slate-900 mb-3 tracking-tight">No products found</h3>
                        <p className="text-slate-500 font-medium">Try adjusting your filters for this category.</p>
                    </div>
                )}
            </div>
        </div>
